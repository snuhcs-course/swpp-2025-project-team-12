...
class ApiIndicesTests(TestCase):
    ...
    def test_indices_kospi_structure(self):
        ...
        if isinstance(kospi, dict):
            ...
            # value는 숫자
            self.assertIsInstance(kospi["value"], (int, float), "kospi.value should be number")

            # changePct도 숫자
            self.assertIsInstance(
                kospi["changePct"], (int, float), "kospi.changePct should be number"
            )
        else:
            # 숫자면 그대로 OK
            self.assertIsInstance(kospi, (int, float), "kospi should be number or dict")
    ...
    def test_indices_kosdaq_structure(self):
        ...
        if isinstance(kosdaq, dict):
            ...
            # value는 숫자
            self.assertIsInstance(kosdaq["value"], (int, float), "kosdaq.value should be number")

            # changePct도 숫자
            self.assertIsInstance(
                kosdaq["changePct"], (int, float), "kosdaq.changePct should be number"
            )
        else:
            # 숫자면 그대로 OK
            self.assertIsInstance(kosdaq, (int, float), "kosdaq should be number or dict")
    ...
    def test_indices_asof_is_timestamp(self):
        ...
        self.assertRegex(asof, r"\d{4}-\d{2}-\d{2}", "asOf should contain date (YYYY-MM-DD)")
    ...
    def test_indices_has_source_field(self):
        ...
        if "source" in data:
            ...
            # "s3" 또는 "mock"
            self.assertIn(source, ["s3", "mock"], "source should be 's3' or 'mock'")
        ...
        for market in ["kospi", "kosdaq"]:
            ...
            if isinstance(market_data, dict) and "changePct" in market_data:
                ...
                with self.subTest(market=market):
                    # 변화율은 대략 -30% ~ +30% 범위
                    self.assertGreaterEqual(change_pct, -30, f"{market} changePct too negative")
                    self.assertLessEqual(change_pct, 30, f"{market} changePct too positive")
