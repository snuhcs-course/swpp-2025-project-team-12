...
class RecommendationBatchModelTests(TestCase):
    ...
    def test_create_batch_minimal(self):
        """최소 필드로 배치 생성"""
        batch = RecommendationBatch.objects.create(market_date=self.market_date)
    ...
    def test_batch_notes_json_field(self):
        """notes JSON 필드"""
        notes_data = {"kospi": 2500, "kosdaq": 800, "overview": "시장 상승세"}

        batch = RecommendationBatch.objects.create(market_date=self.market_date, notes=notes_data)
...
class RecommendationItemModelTests(TestCase):
    ...
    def test_item_ordering(self):
        """rank 순서대로 정렬"""
        item3 = RecommendationItem.objects.create(batch=self.batch, rank=3, ticker="T3", name="N3")
        item1 = RecommendationItem.objects.create(batch=self.batch, rank=1, ticker="T1", name="N1")
        item2 = RecommendationItem.objects.create(batch=self.batch, rank=2, ticker="T2", name="N2")
    ...
    def test_item_related_name(self):
        """batch.items로 역참조 가능"""
        RecommendationItem.objects.create(batch=self.batch, rank=1, ticker="T1", name="N1")
        RecommendationItem.objects.create(batch=self.batch, rank=2, ticker="T2", name="N2")
