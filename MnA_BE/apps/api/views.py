...
class APIView(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                "limit",
                openapi.IN_QUERY,
                description="Number of items (default: 10, max: 100)",
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter(
                "symbol",
                openapi.IN_QUERY,
                description="Specific ticker to retrieve",
                type=openapi.TYPE_STRING,
            ),
        ],
    ...
    def get_company_profiles(self, request: HttpRequest):
        ...
        try:
            ...
            if df is None:
                return degraded(
                    "Profile data not loaded in cache",
                    source="cache",
                    total=0,
                    limit=limit,
                    offset=offset,
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                "ticker",
                openapi.IN_PATH,
                description="Stock ticker code (e.g., 005930)",
                type=openapi.TYPE_STRING,
                required=True,
            ),
        ],
    ...
    def get_reports_detail(self, request: HttpRequest, symbol: str):
        try:
            ...
            if latest_data is not None and len(latest_data) > 0:
                ...
                market_type = (
                    str(row["market"]) if "market" in row and row["market"] is not None else None
                )
                industry = (
                    str(row["industry"])
            ...
            resp = {
                ...
                "industry": industry,
                "price": (
                    {"current": price, "change": change, "change_rate": change_rate}
                ...
                "valuation": valuation,
                "dividend": dividend,
                "financials": {"eps": eps, "bps": bps, "dps": dps, "roe": roe, "div": div},
