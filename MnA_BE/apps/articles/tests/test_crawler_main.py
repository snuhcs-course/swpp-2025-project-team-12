...
class SetupDriverTest(SimpleTestCase):
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_setup_driver_no_chrome_binary(self, mock_exists, mock_run, mock_cdm, mock_service, mock_chrome):
        """Chrome 바이너리 없음"""
        from apps.articles.crawler_main import setup_driver
    ...
    def test_setup_driver_with_env_chrome_bin(self, mock_exists, mock_run, mock_cdm, mock_service, mock_chrome):
        """환경 변수로 Chrome 경로 지정"""
        from apps.articles.crawler_main import setup_driver
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_setup_driver_version_check_exception(self, mock_exists, mock_run, mock_cdm, mock_service, mock_chrome):
        """버전 체크 예외"""
        from apps.articles.crawler_main import setup_driver
...
class ExtractAllContentsTest(SimpleTestCase):
    ...
    def test_extract_all_contents_max_count_limit(self, mock_sleep, mock_content):
        ...
        articles = [
            {"title": f"기사 {i}", "url": f"https://example.com/{i}", "order": i}
            for i in range(10)
        ]
...
class MainFunctionTest(SimpleTestCase):
    ...
    @patch("time.time")
    def test_main_success(self, mock_time, mock_file, mock_makedirs, 
                          mock_setup, mock_crawl, mock_extract, mock_upload):
        """main 함수 성공"""
        ...
        mock_extract.return_value = (
            [{"title": "기사 1", "url": "https://example.com/1", "content": "본문", "content_length": 100}],
            0
        )
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("time.time")
    def test_main_driver_quit_on_exception(self, mock_time, mock_file, mock_makedirs,
                                           mock_setup, mock_crawl, mock_extract, mock_upload):
...
class AdditionalCoverageTest(SimpleTestCase):
    ...
    @patch("subprocess.run")
    @patch("os.path.exists")
    def test_setup_driver_mac_app_path(self, mock_exists, mock_run, mock_cdm, mock_service, mock_chrome):
        """Mac .app 경로 처리"""
        from apps.articles.crawler_main import setup_driver
    ...
    @patch("time.time")
    @patch("sys.stdout")
    def test_main_stdout_reconfigure_exception(self, mock_stdout, mock_time, mock_file, 
                                                mock_makedirs, mock_setup, mock_crawl, 
                                                mock_extract, mock_upload):
