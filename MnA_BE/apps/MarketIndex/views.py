...
class StockIndexView(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                "index_type",
                openapi.IN_PATH,
                description='Index name: "KOSPI", "KOSDAQ", or "BOTH"',
            ...
            openapi.Parameter(
                "days",
                openapi.IN_QUERY,
                description="Number of days to retrieve (default: 30, min: 1, max: 365)",
    ...
    def get_history(self, request, index_type):
        ...
        if index_type.upper() == "BOTH":
            ...
            return JsonResponse(
                {"status": "success", "index": "BOTH", "days": days, "data": both_data}
            )
    ...
    def get_summary(self, request):
        ...
        for index_name, data in summary_data.items():
            formatted_summary[index_name] = {
                ...
                "latest_change_amount": (
                    latest_data[index_name]["change_amount"] if index_name in latest_data else None
                ),
        ...
        return JsonResponse({"status": "success", "data": formatted_summary})
