...
def save_s3_df(now, df_concat):
    bucket = 'swpp-12-bucket'
    s3 = boto3.client("s3")
    buffer = io.BytesIO()
    df_concat.to_parquet(buffer, compression="zstd", index=True)
    buffer.seek(0)
    key = f"price-financial-info-instant/year={now.year}/month={now.month}/{now.strftime('%Y-%m-%d')}.parquet"
...
if is_trading_day_krx():
    ...
    date2 = get_recent_closed_trading_day(2) #가장 최근에서 2번쨰로 종료된 거래일
    print(date1, date2)
    ...
    all_recent_df = pd.concat([recent_kospi, recent_kosdaq]).replace(["nan", "NaN", "None", ""], np.nan).dropna()
