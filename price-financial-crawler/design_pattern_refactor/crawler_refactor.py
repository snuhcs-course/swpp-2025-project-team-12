...
def get_stock_info(biz_day, mktId):  # STK, KSQ
    ...
    sector_stk.columns = [
        ...
        'close', 'change', 'change_rate', 'market_cap'
    ]
...
def save_df_s3(prefix, today, market, df, s3_client: Optional[BaseClient] = None) -> int:
    ...
    key = (
        ...
        f"market={market}/{today.strftime('%Y-%m-%d')}.parquet"
    )
...
if __name__ == "__main__":
    ...
    if trading_day:
        ...
        # S3 저장 (Factory 사용)
        save_df_s3('price-financial-info', today, 'kospi', kospi_df.astype('str'))
        save_df_s3('price-financial-info', today, 'kosdaq', kosdaq_df.astype('str'))
    else:
        print('거래일이 아닙니다')
